...
@section('title', __('general.edt') . ' ' . __('leave.leaveapp'))
@section('stylesheets')
...
@push('breadcrump')
<li class="breadcrumb-item"><a href="{{route('leave.index')}}">{{ __('leave.leaveapp') }}</a></li>
<li class="breadcrumb-item active">{{ __('general.edt') }}</li>
@endpush

@section('content')
<form id="form" action="{{ route('leave.update',['id'=>$leave->id]) }}" class="form-horizontal no-gutters" method="post" enctype="multipart/form-data" autocomplete="off">
  <div class="row">
    {{ csrf_field() }}
    {{ method_field('put') }}
    <div class="col-lg-8">
              ...
              <div class="row">
                <div class="col-sm-6">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="{{ __('employee.employ') }}" id="employee" name="employee" value="{{ $leave->employee->name }}" required>
                  </div>
                </div>
                <div class="col-sm-6">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="Employee NID" id="nid" name="nid" value="{{ $leave->employee->nid }}" readonly>
                  </div>
                </div>
              </div>
              <div class="row">
          ...
          <div class="card card-{{ config('configs.app_theme')}} card-outline">
            <div class="card-header">
              ...
              <div class="pull-right card-tools">
                @if ($leave->status != -1)
                <a href="#" class="btn btn-{{ config('configs.app_theme')}} btn-sm text-white add_leave" data-toggle="tooltip" title="{{ __('general.add') }}" onclick="showList()"><i class="fa fa-plus"></i>
                </a>
                @endif
              </div>
            </div>
                ...
                <table id="leave-list" class="table table-bordered table-striped">
                  <thead>
                    <tr>
                      ...
                      <th style="text-align: center">Attendance ID</th>
                  ...
                  <tbody>
                    ...
                    <tr data-key="{{ $key }}" data-reference_id="{{ $value->reference_id }}" data-date="{{ $value->date }}" data-time_start="{{ $value->start }}" data-time_finish="{{ $value->finish }}" data-type="{{ $value->type }}">
                      <td class="text-center">{{ ($key + 1) }}</td>
                      <td class="text-center">{{ ($value->reference_id) }}</td>
                      ...
                      <td class="text-center">
                        <a href="javascript:void(0);" title="Edit Data"><i class="fa fa-edit" onclick="editLeave(this)"></i></a>
                        @if ($leave->status != -1)
                        / <a href="javascript:void(0);" class="link-red" title="Delete Data"><i class="fa fa-trash" onclick="removeLeave(this)" data-key="{{ $key }}" data-selected_date="{{ $value->date }}"></i></a>
                        @endif
                      </td>
                    </tr>
                    @endforeach
                  ...
                  <tfoot>
                    <tr>
                      <th colspan="2" class="text-right">Total ({{ __('leave.days') }})</th>
                      <th colspan="4" data-total_days="{{ $leave->duration }}">{{ $leave->duration }} {{ __('leave.days') }}</th>
                    </tr>
                  </tfoot>
                </table>
              </div>
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-sm-12">
              <div class="form-group">
                <label>Status</label>
                <input type="text" class="form-control" placeholder="Status" id="status" name="status" readonly @if ($leave->status == 2)
                value="Rejected"
            ...
            <div class="col-sm-12">
              <div class="form-group">
                ...
                <input type="text" class="form-control" placeholder="{{ __('leave.leavetp') }}" id="leave_type" name="leave_type" @if ($leave->leavesetting) value="{{ $leave->leavesetting->id }}" @endif>
              </div>
            </div>
            <div class="col-sm-12">
              <div class="form-group">
            ...
            <div class="col-sm-12">
              <div class="form-group">
                ...
                <textarea class="form-control" id="notes" name="notes" rows="5" placeholder="{{ __('general.notes') }}">{{ $leave->notes }}</textarea>
              </div>
            </div>
          </div>
        </div>
    ...
    $('#form').validate({
      ...
      submitHandler: function() {
        ...
        $.each($('#leave-list > tbody').find('tr[data-date]'), function (key, value) {
          formData.append('reference_id[]', $(value).data('reference_id'));
