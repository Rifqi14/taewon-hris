...
@push('breadcrump')
<li class="breadcrumb-item"><a href="{{ route('allowanceconfig.index') }}">Allowance Config</a></li>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label for="workgroup_id" class="control-label">Workgroup <b class="text-danger">*</b></label>
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <input type="text" name="allowance_id" id="allowance_id" class="form-control" data-placeholder="Select Allowance" required>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                ...
                <select name="type" id="type" class="form-control select2" data-placeholder="Select Type" required>
                  ...
                  <option value="{{ $key }}"@if ($allowanceconfig->type==$key) selected @endif>{{ $item }}</option>
                  @endforeach
                </select>
              </div>
            </div>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $("#workgroup_id").select2('data',{id:{{$allowanceconfig->workgroup_id}},text:'{{$allowanceconfig->workgroup->workgroup->name}}'}).trigger('change');

    $(document).on("change", "#workgroup_id", function () {
        var workgroup_id = $(this).select2('data').workgroup_id;
        $('#workgroup').attr("value",workgroup_id);
      if (!$.isEmptyObject($('#form').validate().submitted)) {
        $('#form').validate().form();
      }
      $('#allowance_id').select2('val','');
    });
     $("#allowance_id").select2('data',{id:{{$allowanceconfig->allowance_id}},text:'{{$allowanceconfig->allowance->allowance->allowance}}'}).trigger('change');
