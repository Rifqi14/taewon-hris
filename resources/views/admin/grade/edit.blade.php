...
@section('title', __('general.edt') . ' ' . __('grade.grade'))
@section('stylesheets')
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="card card-{{ config('configs.app_theme') }} card-outline">
					...
					<div class="card-body">
						<form id="form" action="{{ route('grade.update', ['id'=>$grade->id])}}" method="post">
							{{ csrf_field() }}
							@method('put')
							<div class="row">
								<div class="col-sm-6">
									...
									<div class="form-group">
										...
										<input type="text" class="form-control" placeholder="{{ __('general.code') }}" name="code" value="{{ $grade->code }}">
									</div>
								</div>
								<div class="col-sm-6">
									<div class="form-group">
										<label>{{ __('general.name') }}</label>
										<input type="text" name="name" class="form-control" placeholder="{{ __('general.name') }}" value="{{ $grade->name }}">
									</div>
								</div>
							</div>
							<div class="row">
								<div class="col-sm-6">
									...
									<div class="form-group">
										...
										<input type="text" class="form-control" placeholder="{{ __('grade.order') }}" name="order" value="{{ $grade->order }}" readonly>
									</div>
								</div>
								<div class="col-sm-6">
									<div class="form-group">
										...
										<div class="row">
											<div class="col-sm-4">
												<select class="form-control select2" name="min_duration" id="min_duration">
													<option value="yes" @if($grade->min_duration == 'yes') selected @endif>{{ __('general.yes') }}
													</option>
													<option value="no" @if($grade->min_duration == 'no') selected @endif>{{ __('general.no') }}
													</option>
												</select>
											</div>
											<div class="col-sm-8">
												<input type="number" id="month" name="month" class="form-control" value="{{ $grade->month }}" placeholder="{{ __('general.month') }}" @if($grade->min_duration == 'no') readonly @endif>
											</div>
										</div>
									</div>
								</div>
						...
						<div class="row">
							<div class="col-sm-6">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="bestsallary_id" name="bestsallary_id" data-placeholder="Select {{ __('general.chs') . ' ' . __('grade.rgnslr') }}">
								</div>
							</div>
							<div class="col-sm-6">
								<div class="form-group">
									<label>{{ __('grade.slrvalue') }}</label>
									<input type="text" name="basic_umk_value" class="form-control" readonly="" placeholder="{{ __('grade.slrvalue') }}" value="{{ $grade->basic_umk_value }}">
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-sm-6">
								...
								<div class="form-group">
									...
									<select class="form-control select2" name="additional_type" id="additional_type">
										<option></option>
										<option value="percentage" @if($grade->additional_type == 'percentage') selected
											@endif>Percentage</option>
										<option value="nominal" @if($grade->additional_type == 'nominal') selected
											@endif>Nominal
										</option>
										<option value="none" @if($grade->additional_type == 'none') selected @endif>None
										</option>
									</select>
								</div>
							</div>
							<div class="col-sm-6">
								<div class="form-group">
									...
									<input type="text" id="additional_value" class="form-control" placeholder="{{ __('grade.addvalue') }}" name="additional_value" value="{{ $grade->additional_value }}">
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-sm-12">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" placeholder="{{ __('grade.totalslr') }}" name="basic_sallary" value="{{ $grade->basic_sallary }}" readonly="">
								</div>
							</div>
						</div>
					</div>
				...
				<div class="row">
					<div class="col-sm-12">
						...
						<div class="form-group">
							...
							<textarea class="form-control" style="height: 100px;" name="notes" placeholder="{{ __('general.notes') }}">{{ $grade->notes }}</textarea>
						</div>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-12">
						<div class="form-group">
							...
							<select class="form-control select2" id="status" name="status" data-placeholder="{{ __('general.chs') . ' ' . __('general.status') }}">
								<option></option>
								<option value="1" @if($grade->status == '1') selected @endif>{{ __('general.actv') }}</option>
								<option value="0" @if($grade->status == '0') selected @endif>{{ __('general.noactv') }}</option>
							</select>
						</div>
					</div>
					</form>
...
<script>
	...
	$(document).ready(function(){
		...
		$("#bestsallary_id").select2('data',{id:{{$grade->bestsallary->id}},text:'{{$grade->bestsallary->region->name}}'}).trigger('change');
