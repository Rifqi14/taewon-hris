...
<div class="card card-default">
  <div class="card-header">
    <h3 class="card-title">Ubah Kota</h3>
    <!-- tools card -->
  ...
  <div class="card-body">
      <form id="form" action="{{route('region.update',['id'=>$region->id])}}" class="form-horizontal" method="post" autocomplete="off">
          {{ csrf_field() }}
          ...
          <div class="box-body">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-6">
              <input type="text" class="form-control" id="name" name="name" placeholder="Nama" value="{{$region->name}}" required>
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="col-sm-6">
                  <select class="form-control select2" id="type" name="type" required>
                      <option value="Kabupaten" @if($region->type == 'Kabupaten') selected @endif>Kabupaten</option>
                      <option value="Kota" @if($region->type == 'Kota') selected @endif>Kota</option>
                  </select>
              </div>
            </div>
          </div>
...
<script>
  $(document).ready(function(){
      ...
      $("#province_id").select2('data',{id:{{$region->province->id}},text:'{{$region->province->name}}'}).trigger('change');
