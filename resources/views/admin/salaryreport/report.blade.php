...
<body onload="window.print();">
	...
	@foreach ($report->salarydetail as $item)
		@if ($item->type == 0)
	...
	<div class="container">
		...
		{{-- </div> --}}
		<hr>
		<h5 class="text-right">{{ changeDateFormat('F Y', $report->period) }}</h5>
		<div class="employee-section">
			<table class="employee-table">
				<tbody>
					<tr>
						<td style="width: 5%">Name</td>
						<td style="width: 25%">: {{ $report->employee->name }}</td>
						<td style="width: 5%">NPWP</td>
						<td style="width: 25%">: {{ $report->employee->npwp }}</td>
						
					</tr>
					<tr>
						<td style="width: 5%">NIK</td>
						<td style="width: 25%">: {{ $report->employee->nid }}</td>
						<td style="width: 5%">Status</td>
						<td style="width: 25%">: {{ $employee->workgroup->name }}</td>
					</tr>
					<tr>
						<td style="width: 5%">Join Date</td>
						<td style="width: 25%">: {{ $employee->join_date}}</td>
						<td style="width: 5%">Department</td>
						<td style="width: 25%">: {{ $employee->department->name }}</td>
					</tr>
					<tr>
						<td style="width: 5%">Position</td>
						<td style="width: 25%">: {{ $employee->title->name }}</td>
					</tr>
				</tbody>
			</table>
		</div>
		<table class="gross-table" style="padding-top: 20pt">
			...
			<tbody>
				@foreach ($report->salarydetail as $key => $item)
					@if ($item->type == 1)
			...
			<tfoot>
				<tr>
					...
					<td style="padding-right: 20pt;" class="header-slip net-salary">{{ number_format($report->gross_salary, 0, '.', ',') }}</td>
				</tr>
				<tr>
					<td class="net-total">NET SALARY</td>
					<td style="padding-right: 20pt;" class="net-amount">{{ number_format($report->net_salary, 0, '.', ',') }}</td>
				</tr>
				<tr>
		...
		<table class="deduction-table" style="padding-top: 20pt">
			...
			<tfoot>
				<tr>
					...
					<td class="header-slip deduction-salary">{{ number_format($report->deduction, 0, '.', ',') }}</td>
				</tr>
			</tfoot>
		</table>
	</div>
