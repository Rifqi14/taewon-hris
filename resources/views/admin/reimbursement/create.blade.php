...
@section('content')
<form id="form" action="{{ route('reimbursement.store') }}" class="form-horizontal" method="post" autocomplete="off">
  <div class="row">
    <div class="col-lg-12">
              ...
              <div class="form-group col-sm-6">
                <div class="row">
                  ...
                  <div class="col-sm-8 controls">
                    ...
                    <input type="hidden" name="dailyreportdriver_id" id="dailyreportdriver_id">
                    <input type="hidden" name="max_arrival" id="max_arrival">
                    <input type="hidden" name="get_day" id="get_day">
      ...
      <div class="card card-{{ config('configs.app_theme') }} card-outline">
        ...
        <div class="card-body">
          <table class="table table-striped table-bordered datatable" id="table-product" style="width: 100%">
            ...
            <input type="hidden" name="subtotalallowance" class="form-control form-control-sm currency text-right"  value="0">
...
<script type="text/javascript">
  ...
  function selectCalculation(that)
  {
    ...
    var total = $(that).data('total');
     if ($.inArray(reff_detail, so_ids) != -1) {
          $.gritter.add({
              title: 'Warning!',
  ...
  function selectAdditional(that)
  {
    ...
    var total = $(that).data('total');
    var description = $(that).data('description');
     if ($.inArray(reff_additional, so_ids) != -1) {
          $.gritter.add({
              title: 'Warning!',
              text: 'Data already selected',
              class_name: 'gritter-warning',
  ...
  $(document).ready(function(){
    ...
    $(document).on("change", "#pick_id", function() {
        so_ids = [];
        $('#get_day').val("");
    ...
    $('#table-product').on('click', '.remove', function () {
			var id = $(this).parents('tr').data('reff_detail_additional');

			for (let i = 0; i < so_ids.length; i++) {
				if (so_ids[i] === id) {
					so_ids.splice(i, 1);
				}
			}
